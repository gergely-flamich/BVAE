...
class GaussianDenseWithGammaPrior(tf.keras.layers.Dense):
    ...
    def resample_precisions(self):
        ...
        kernel_precision = self.scale_to_prec(self.kernel_scale)

        if self.use_bias:
            bias_precision = self.scale_to_prec(self.bias_scale)

        # Perform Gibbs step for appropriate prior mode
        if self.prior_mode == "per_param":

            kernel_conc = self.alpha0 + 0.5
            kernel_conc = tf.ones_like(kernel_precision) * kernel_conc
            kernel_rate = self.beta0 + tf.square(kernel_precision) / 2.

            if self.use_bias:
                bias_conc = self.alpha0 + 0.5
                bias_conc = tf.ones_like(bias_precision) * bias_conc
                bias_rate = self.beta0 + tf.square(bias_precision) / 2.

        elif self.prior_mode == "weight_and_bias":

            kernel_conc = self.alpha0 + tf.cast(tf.size(kernel_precision), self.kernel.dtype) / 2.
            kernel_rate = self.beta0 + tf.reduce_sum(tf.square(kernel_precision)) / 2.

            if self.use_bias:
                bias_conc = self.alpha0 + tf.cast(tf.size(bias_precision), self.bias.dtype) / 2.
                bias_rate = self.beta0 + tf.reduce_sum(tf.square(bias_precision)) / 2.

        elif self.prior_mode == "per_unit":

            conc = self.alpha0 + tf.cast(kernel_precision.shape[0], self.kernel.dtype) / 2.
            rate = self.beta0 + tf.reduce_sum(tf.square(kernel_precision), axis=0) / 2.

            if self.use_bias:
                conc = conc + 0.5
                rate = rate + tf.square(bias_precision) / 2.

                bias_conc = tf.ones_like(bias_precision) * conc
                bias_rate = rate
        ...
        if self.use_bias:
            ...
            # Note that this is only assigned here so that the dictionary below can be constructed
            new_bias_scale = tf.zeros(self.units)
