...
class GaussianConv2DTransposeWithPrior(tf.keras.layers.Conv2DTranspose):
    _AVAILABLE_PRIOR_MODES = [
        "per_param",
        "weight_and_bias",
        "per_filter"
    ...
    def resample_precisions(self):
        ...
        if self.prior_mode == "per_param":
            ...
            conc = self.alpha0 + tf.cast(n_elems_per_filter, self.kernel.dtype) / 2.
            rate = self.beta0 + tf.reduce_sum(tf.square(self.kernel), axis=[0, 1, 3]) / 2.
        ...
        if self.prior_mode == "per_param":
            ...
            assigned_new_kernel_scale = tf.ones(self.kernel.shape[:-2] + [1] + self.kernel.shape[-1:]) * new_kernel_scale[None, :]
        elif self.prior_mode == "weight_and_bias":
            assigned_new_kernel_scale = tf.ones_like(self.kernel) * new_kernel_scale
        else:
            raise NotImplementedError
